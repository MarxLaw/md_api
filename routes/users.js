...
router.put("/register", async function (req, res, next) {
  try {
    ...
    await mysqlConnection
      .promise()
      .query(`UPDATE ${tableName} SET account_id = ? WHERE id = ?`, [
        accountId,
        pkId,
      ]);
...
router.post("/forgot-password", async (req, res) => {
  ...
  const [users] = await mysqlConnection
    .promise()
    .query("SELECT id FROM caregiver_account WHERE email = ?", [email]);
...
router.post("/verify-otp", async (req, res) => {
  ...
  const [resets] = await mysqlConnection
    .promise()
    .query(
...
router.post("/reset-password", async (req, res) => {
  ...
  await mysqlConnection
    .promise()
    .query("UPDATE caregiver_account SET password = ? WHERE id = ?", [
      hashedPassword,
      reset.user_id,
    ]);
  ...
  await mysqlConnection
    .promise()
    .query("DELETE FROM password_reset WHERE id = ?", [reset.id]);
